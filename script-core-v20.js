(() => {
  ...
  const applyLogo = src => {
    ...
    if (favicon) {
      favicon.href = src;
      favicon.type = src.includes('.svg') ? 'image/svg+xml' : 'image/jpeg';
    }
    const ogImage = document.querySelector('meta[property="og:image"]');
    if (ogImage) ogImage.content = 'https://elmaq-engenharia-site.vercel.app/' + src.split('?')[0] + '?v=20';
  };

  const tryLogo = index => {
    if (index >= logoCandidates.length) { applyLogo(fallbackLogo); return; }
  ...
  document.addEventListener('keydown', e => {
    if (e.key === 'Escape') { setMenu(false); closeGallery(); }
    if (!modal?.classList.contains('open')) return;
